...
Public Function GetVideoURL(numJug As String, combo As String) As String
    ...
    If Not wsExp Is Nothing Then
        ...
        If hojaRival <> "" Then
            ' C6 ya tiene el nombre completo de la hoja de videos (ej: VIDEOS_LOMAS)
            hojaVideos = hojaRival
        End If
    End If
...
Public Sub GenerarGamePlanJS()
    ...
    ' Leer jugadores desde PLANTILLA_RIVAL (cols B=num, C=nombre, D=posicion)
    Dim tacInfo(19, 9) As String
    Dim ri As Integer
    Dim wsPlan As Worksheet
    On Error Resume Next
    Set wsPlan = ThisWorkbook.Sheets("PLANTILLA_RIVAL")
    On Error GoTo 0
    If wsPlan Is Nothing Then
        MsgBox "No se encontro PLANTILLA_RIVAL", vbExclamation
        Exit Sub
    End If
    Dim tiCount As Integer: tiCount = 0
    Dim rp As Integer
    For rp = 6 To 25
        Dim vNum As Variant: vNum = wsPlan.Cells(rp, 2).Value  ' Col B = numero
        Dim vNom As Variant: vNom = wsPlan.Cells(rp, 3).Value  ' Col C = nombre
        Dim vPos As Variant: vPos = wsPlan.Cells(rp, 4).Value  ' Col D = posicion
        If IsEmpty(vNum) Or IsNull(vNum) Or CStr(vNum) = "" Then GoTo SigPlan
        If IsEmpty(vNom) Or IsNull(vNom) Or CStr(vNom) = "" Then GoTo SigPlan
        tacInfo(tiCount, 0) = Trim(CStr(vNum))  ' numero
        tacInfo(tiCount, 1) = Trim(Replace(CStr(vNom), Chr(160), " "))  ' nombre completo
        tacInfo(tiCount, 2) = Trim(CStr(vPos))  ' posicion
        ' Resto de campos (bloqueo, defensa, etc) vacios por ahora
        tacInfo(tiCount, 3) = ""
        tacInfo(tiCount, 4) = ""
        tacInfo(tiCount, 5) = ""
        tacInfo(tiCount, 6) = ""
