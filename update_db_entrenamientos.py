...
def parse_dvw_both(fpath, temporada):
    ...
    for team, pfx, section in [(home,'*','[3PLAYERS-H]'),(away,'a','[3PLAYERS-V]')]:
        ...
        team = 'ENTRENAMIENTO'  # ENTRENAMIENTO: todo San Lorenzo en un solo equipo
        ...
        if team in result:
            # Acumular el segundo lado (away) sobre el primero (home)
            r=result[team]
            for k,v in players.items():
                if k not in r['players']: r['players'][k]=v
            for src_d,dst_k in [(atk,'atk'),(srv,'srv'),(rec,'rec'),(sets,'sets'),(blk,'blk')]:
                for num,acts in src_d.items():
                    r[dst_k].setdefault(num,[]).extend(acts)
        else:
...
# ── UPDATE DATABASE ───────────────────────────────────────────────
def update_database(dvw_dir, temporada, db_path='entrenamientos_db.json'):
    # Load existing DB
...
def build_stats_table(players, teams, output_path='nla_stats_table.html'):
    """Genera stats_entrenamiento.js con los datos de entrenamiento (para el selector)."""
    out_dir = os.path.dirname(output_path) or '.'
    data_path = os.path.join(out_dir, 'stats_entrenamiento.js')
    js = "window.STATS_ENT = {\n"
    js += "  players: " + json.dumps(players, ensure_ascii=False) + ",\n"
    js += "  teams: " + json.dumps(teams, ensure_ascii=False) + "\n};\n"
    with open(data_path, 'w', encoding='utf-8') as f:
        f.write(js)
    print(f"   \u2713 stats_entrenamiento.js: {os.path.getsize(data_path)//1024}KB ({len(players)} jugadores)")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--db_path',    default='entrenamientos_db.json', help='Database file path')
