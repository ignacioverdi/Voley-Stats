#!/usr/bin/env python3
# CASLA VOLEY — generar_datos_entrenamientos.py
# Lee CASL_Entrenamientos.xlsx y genera datos_entrenamientos.js
...
EXCEL = os.path.join(BASE, "CASL_Entrenamientos.xlsx")
OUT   = os.path.join(BASE, "datos_entrenamientos.js")
SKIP  = {'INDICE', 'GUIA', 'INSTRUCCIONES', 'PRÓXIMO', 'PLANTEL', 'CASLA'}
...
def leer_indice(wb):
    ...
    for row in ws.iter_rows(min_row=3, max_row=50, values_only=True):
        ...
        meta[solapa] = {
            ...
            'descripcion': str(row[2]).strip() if row[2] else solapa,
...
def leer_objetivos_acumulados():
    gameplan = EXCEL  # Use CASLA sheet from same file
    if not os.path.exists(gameplan):
        print("  AVISO: No se encontró el Excel para objetivos acumulados")
        return {}, {}
    try:
        # For entrenamientos, acumulado is in the CASLA sheet of the same file
        wb_gp = load_workbook(EXCEL, read_only=True, data_only=True, keep_vba=False)
        ws = wb_gp['CASLA']
    except Exception as e:
...
def main():
    print("="*50)
    print("CASLA VOLEY — Generar datos_entrenamientos.js")
    print("="*50)
    ...
    js = f"""// datos_entrenamientos.js — {generado}
const ENTRENAMIENTOS_GENERADO = "{generado}";
const ENTRENAMIENTOS_TOTAL = {len(partidos_procesados)};
const ENTRENAMIENTOS_META = {json.dumps(meta_list, ensure_ascii=False, indent=2)};
const ENTRENAMIENTOS_JUGADORES = {json.dumps(jugadores, ensure_ascii=False, indent=2)};

const ENTRENAMIENTOS_ARMADOR = {json.dumps(all_armadores, ensure_ascii=False, indent=2)};
const ENTRENAMIENTOS_INDIVIDUAL = {json.dumps(partidos_list, ensure_ascii=False, indent=2)};

// Objetivos del equipo acumulado
const ENTRENAMIENTOS_EQUIPO_OBJ = {json.dumps(equipo_obj, ensure_ascii=False)};
"""
    with open(OUT,'w',encoding='utf-8') as f: f.write(js)
    print(f"\n✓ datos_entrenamientos.js generado ({len(js)//1024}KB) — subir a GitHub")
