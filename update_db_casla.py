...
# ── NOMBRES MANUALES (override) ───────────────────────────────────
# Corrige nombres que vienen mal o vacíos desde los .dvw. Se aplican CADA
# vez que se regenera, así NO se pierden al cargar partidos nuevos.
# Formato:  'Equipo': { numero: 'Apellido Nombre' }
NAME_OVERRIDES = {
    'River': {9: 'Mendez Nicolas', 10: 'Trucco Federico', 11: 'Fernandez Gaston'},
}
def override_name(team, num, default_name=''):
    try: num = int(num)
    except: return default_name
    ov = NAME_OVERRIDES.get(team) or NAME_OVERRIDES.get(str(team).lower().replace(' ', '_'))
    if ov and num in ov: return ov[num]
    return default_name
...
def parse_dvw_both(fpath, temporada):
    ...
    for team, pfx, section in [(home,'*','[3PLAYERS-H]'),(away,'a','[3PLAYERS-V]')]:
        ...
        for _pn, _pinfo in players.items():
            _nw = override_name(team, _pn, '')
            if _nw:
                _pinfo['name'] = _nw
                _pinfo['apellido'] = _nw.split(' ')[0]
...
def build_liga_data(teams_data, combos, output_dir='.', setters=None, rallies=None):
    ...
    for team in NLA_TEAMS:
        ...
        for sn in team_setters:
            ...
            if not rl: continue
            sname = override_name(team, sn, td.get(str(sn),{}).get('info',{}).get('name',f'#{sn}'))
...
def generate_team_pages_data(dvw_dir, team_name, output_dir='.', temporada='2025/26'):
    ...
    partidos_meta=[{'id':g['rival']+'__'+g['date'],'nombre':g['rival'],'rival':g['rival'],'fecha':'/'.join(reversed(g['date'].split('-'))),'torneo':f'{LIGA_NOMBRE} {temporada}','resultado':g['result'],'sets_nafels':str(g['tsets']),'sets_rival':str(g['rsets'])} for g in sorted(games,key=lambda x:x['date']) if g['date']]
